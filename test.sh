...
docker-compose up -d && docker logs dockersvn_svnsync_1

# this will leave only the backup repo running
...
# this will leave only the master running
docker kill dockersvn_svnbackup_1 && docker-compose rm -f
docker-compose up -d && docker logs dockersvn_svnsync_1

docker exec dockersvn_svnslave_1 repocreate r1 -o daemon -u 3818ff11-c409-4f9e-acd1-1d9903fc4329 -c 1
docker-compose up -d && docker logs dockersvn_svnsync_1
